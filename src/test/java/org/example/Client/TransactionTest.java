...
public class TransactionTest {
    ...
    @Test
    public void TestToAuctionGrpc2() {
        var key = KeysManager.generateKeys();

        Auction a = new Auction(new byte[]{1, 2, 3}, 1, 1, 1, key.getPublic(), new byte[]{1, 2, 3}, new byte[]{1, 2, 3});

        Auction b = (Auction) Transaction.fromGrpc(a.toGrpc(new byte[]{1}));

        assert a.equals(b);
    }

    @Test
    public void TestToBidGrpc2() {
        var key = KeysManager.generateKeys();
        ...
        Bid b = (Bid) Transaction.fromGrpc(a.toGrpc(new byte[]{1}));
